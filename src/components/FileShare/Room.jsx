...
export default function Room() {
  ...
  useEffect(() => {
    ...
    socket.onopen = () => {
      socket.send(JSON.stringify({ type: "join", room: id }));
    };
    ...
    socket.onmessage = (msg) => {
      ...
      if (data.type === "start") {
        peerRef.current = createPeer(true, socket);
      ...
      <div className="max-w-lg mx-auto flex flex-col gap-4">
        ...
        <div className="bg-zinc-900 rounded-2xl p-5">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="text-[11px] text-gray-400 uppercase tracking-widest mb-1">Room</p>
              <div className="flex items-center gap-2">
                <span className="text-lg font-medium font-mono text-gray-300">{id}</span>
                <button
                  onClick={copyRoomId}
                ...
                >
                  {copied ? (
                    <>
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12" strokeLinecap="round" strokeLinejoin="round" /></svg>
                      Copied
                    </>
                  ) : (
                    <>
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" strokeLinecap="round" /></svg>
                      Copy
                    </>
                  )}
                </button>
            ...
            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${status === "connected" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${status === "connected" ? "bg-green-500" : "bg-amber-400"}`} />
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex gap-6">
            <div>
              ...
              <p className="text-sm font-medium text-gray-300">{status === "connected" ? "2 / 2" : "1 / 2"}</p>
            </div>
            <div>
        ...
        <div className="bg-zinc-900 rounded-2xl p-5 flex flex-col gap-4">
          ...
          {!file ? (
            ...
            >
              ...
              <p className="text-xs text-gray-300">Any file type · No size limit</p>
            ...
            <div className="flex items-center gap-3 bg-gray-900 rounded-xl px-3 py-2.5">
              <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
                <svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
              ...
              <button onClick={() => setFile(null)} className="text-gray-300 hover:text-gray-500 p-1">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            </div>
          )}
        ...
        <div className="bg-zinc-900 rounded-2xl p-5">
          ...
          {received.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {received.map((f, i) => (
                <div key={i} className="flex items-center gap-3 bg-zinc-900 rounded-xl px-3 py-2.5">
                  <div className="w-8 h-8 rounded-lg bg-green-50 flex items-center justify-center shrink-0">
                    <svg className="w-3.5 h-3.5 text-green-600" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </div>
                  <div className="flex-1 min-w-0">
